...
export default function ContainerPickup({
  ...
  const [formData, setFormData] = useState({
    appointmentTime: '2025-12-20T14:55',
    deliveryMethod: 'yard',
    remarks: '',
  ...
  const handleSubmit = async () => {
    ...
    if (formData.deliveryMethod === 'direct') {
      if (!formData.destinationContact) {
        toast.error('请输入收货联系人')
        return
      }
      if (!formData.destinationPhone) {
        toast.error('请输入收货联系电话')
        return
      }
      if (!formData.destinationAddress) {
        toast.error('请输入收货详细地址')
        return
      }
    }
    ...
    try {
      // 调用API提交预约数据
      await request.post('/bzss/api/Containers/AddContainerPickup', {
        containerNo: deliveryItem?.containerNo || containerNo,
        pickupCode: deliveryItem?.pickupCode || pickupCode,
        containerType: deliveryItem?.containerType || containerType,
        ...formData,
      })
      ...
      toast.error(error?.response?.data?.message || '预约提柜失败')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200 bg-white">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-lg font-semibold">预约提柜</h1>
        </div>
      ...
      <div className="px-6 py-6 space-y-8">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex items-center gap-4">
              <Label className="w-28 text-right font-medium">还柜联系人</Label>
              <Input value={formData.returnContact} onChange={(e) => handleInputChange('returnContact', e.target.value)} className="h-9 flex-1" />
            </div>
            <div className="flex items-center gap-4">
              <Label className="w-28 text-right font-medium">还柜联系电话</Label>
              <Input value={formData.returnPhone} onChange={(e) => handleInputChange('returnPhone', e.target.value)} className="h-9 flex-1" />
            </div>
          </div>
          <div className="flex items-center gap-4">
            <Label className="w-28 text-right font-medium">还柜详细地址</Label>
            <Input value={formData.returnAddress} onChange={(e) => handleInputChange('returnAddress', e.target.value)} className="h-9 flex-1" />
          </div>
        </div>
