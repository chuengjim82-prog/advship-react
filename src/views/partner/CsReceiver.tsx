...
const csReceiverSchema = z.object({
...
export default function City() {
  ...
  const formRef = useRef<UseFormReturn<CsReceiverData> | null>(null)
  ...
  const columns: ColumnDef<CsReceiverData>[] = [
      ...
      <>
        ...
        <FormField control={form.control} name="cityName" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Input placeholder="请输入城市" {...field} /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
  ...
  const defaultValues: CsReceiverData = {
    statusi: 1,
    statuss: '',
    cityName: '',
    contact: '',
  ...
  return (
    <>
      <CrudTableV2<CsReceiverData>
        title="客户收件人"
        apiUrl="/base/api/csReceiver"
        columns={columns}
        formSchema={csReceiverSchema}
