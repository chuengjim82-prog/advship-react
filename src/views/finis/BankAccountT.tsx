...
export default function BankAccountT() {
  ...
  const columns: ColumnDef<BankAccountTData>[] = [
    { accessorKey: 'id', header: '主键', size: 60 },
    {
      ...
      header: () => <span className="flex items-center gap-1.5"><Building2 className="h-3.5 w-3.5 text-primary" />账户类型</span>,
      size: 100,
      cell: ({ getValue }) => {
    ...
    { 
      ...
      header: () => <span className="flex items-center gap-1.5"><CreditCard className="h-3.5 w-3.5 text-muted-foreground" />账户名称</span>,
      size: 180,
    },
    { 
