...
export default function Sidebar({ collapsed }: SidebarProps) {
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      children: [
        ...
        { key: '/basic/feeType', icon: 'PercentageOutlined', label: t('menu.feeType'), path: '/basic/feeType' },
        { key: '/basic/feeItem', icon: 'FileTextOutlined', label: t('menu.feeItem'), path: '/basic/feeItem' }
      ]
    },
    {
      key: 'partner',
    ...
    },
    {
      ...
      children: [
        { key: '/code-generator', icon: 'SettingOutlined', label: t('menu.codeGenerator'), path: '/code-generator' }
      ]
    }
  ]
  ...
  const renderMenuItem = (item: MenuItem) => {
            ...
            <div className="bg-[#1f2d3d]">
              {item.children!.map(child => {
                ...
                return (
                  <div
                    key={child.key}
                    className={`flex h-10 cursor-pointer items-center gap-3 pl-12 pr-4 text-sm transition-colors hover:bg-[#263445] ${
                      isChildActive ? 'bg-[#263445] text-[#409eff]' : 'text-[#bfcbd9]'
                    }`}
    ...
    return (
      <div
        key={item.key}
        className={`flex h-10 cursor-pointer items-center gap-3 px-4 transition-colors hover:bg-[#263445] ${
          isActive ? 'bg-[#263445] text-[#409eff]' : 'text-[#bfcbd9]'
        }`}
  ...
  return (
    <div className="flex h-screen flex-col bg-sidebar">
      ...
      <div className="flex h-[60px] shrink-0 items-center justify-center bg-[#2b3649]">
        ...
        {!collapsed && (
          <span className="ml-3 text-lg font-bold text-white">AdvShip</span>
        )}
      </div>
